     				...
     				if ($arc){
     					...
     					//if ($User->CheckPermission ('ESN') or '1' == $mss_perm ){
    						...
    						//$result=$User->GetUserInfo();
     						...
     						session('user.dept_seqno','$result.DEPT_SEQNO');     						session('user.dept_id','$result.DEPT_ID');
    						...
    						session('user.is_manager','$mss_perm');
     						...
     						var_export($_SESSION);
     						$this->success('登录成功，跳转至主页',"/think/index.php/Home/Redirect/index",'3');
     						exit();
     					}else {
     						header('think/index.php/Home/Index/index'.urldecode('未授权'));
     						exit();
     					}
    				}else {
     					//echo '1';
     					$this->error('think/index.php/Home/Index/index'.urldecode('Password error.'));
     					exit();
     				}
     			}else {

    				$this->error('think/index.php/Home/Index/index'.urldecode('The user name does not exist.'));
    				exit();
    			}
    		}else {
    			header('think/index.php/Home/Index/index'.urldecode('Attack error.'));
    			exit();
    		}
    	}
    }
