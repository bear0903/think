...
switch($a)
{
	...
	case 'query':
		...
		$room_emp_list = $Dorm->getRoomEmpList($_POST['check_date'], 
											   $_POST['check_times'],
											   $_POST['area_code'], 
											   ...
											   $_POST['room_no']);
		//pr($room_emp_list);
		$g_parser->ParseTable('emp_list', $room_emp_list);
		break;
	case 'save':
		...
		$r = $Dorm->saveRoomCheckResult($_POST['room_bed_master_seq'][0],$_SESSION['user']['emp_seq_no'], $_SESSION['user']['emp_id'], $_SESSION['user']['emp_name'], $rows);
		if ($r>0) showMsg('查房资料保存成功','success');
		break;
	default:
...
if(isset($_POST['area_code'])){
	...
	$g_parser->ParseSelect('bed_status_list',$Dorm->getBedStatusList(),'','');
}
