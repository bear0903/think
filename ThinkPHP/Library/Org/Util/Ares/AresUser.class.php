 ...
 *  $Rev: 3819 $
 *  $Date: 2014-08-12 16:42:07 +0800 (鍛ㄤ簩, 12 鍏湀 2014) $
 *  $Author: dennis $
 *  $LastChangedDate: 2014-08-12 16:42:07 +0800 (鍛ㄤ簩, 12 鍏湀 2014) $
 \****************************************************************************/
class AresUser
{
    // private variables
...
eof;
   ...
   /**
    * 妫�鏌ュ綋鍓� Login User鏄笉鏄富绠� (浠讳綍涓�涓儴闂ㄧ殑涓荤,鍗充笉鏄湰閮ㄩ棬鐨勪富绠′篃鍙互)
    *
    * @param string $user_emp_seq_no
    * @return boolean true_鏄儴闂ㄤ富绠� false_闈為儴闂ㄤ富绠�
    * @author Dennis 2008-09-17 rewrite
    */
    /*
    function IsManager($user_emp_seq_no)
	{
		// modify by Dennis 2009-03-04, 鍙栨秷鏄富鎵嶈兘login MSS 鐨勯檺鍒�
		return true;
		//$this->_dBConn->debug = true;
		// 鍙鏄煇涓儴闂ㄧ殑涓荤灏卞彲浠�
        $sql = <<<eof
...
eof;
		...
		//get_real_ip()寰楀埌瀹㈡埛鐪熸ip鍦板潃 libs/functions.php閲岄潰瀹氫箟
		$addr = get_real_ip();
...
eof;
	...
	/**
    *   @desc  鍒ゆ柇鏄笉鏄憳宸ワ紝浣跨敤鑰呮。鐨勪娇鐢ㄨ�呭彲浠ヤ笉瀵瑰簲鍛樺伐锛屼絾鏄笉瀵瑰簲鍛樺伐灏变笉鍙互鐧诲綍 ESS
    *   @param no parameter
...
eof;
	...
	*/
	function GetMenu($user_seqno,$sys_name,$lang)
	{
		...
		/* remark by dennis 2011-08-02 鏈敤鍒颁互涓嬬殑 temporary table
...
eof;
		...
		switch ($do){
			case 'rootDeptAllBelow':
				// 璩囨枡娆婇檺瑷畾涓矑鏈夎ō瀹氶儴闁�娆婇檺锛岃嫢姝や汉鏄儴闁�涓荤,闋愯ō椤ず鏈儴闁�
				$sql = <<<eof
...
eof;
				...
				break;
			case 'dept':
				 // 鏍规摎 Root Node 涓嬮’绀虹殑閮ㄩ杸鍘诲彇涓嬮殠閮ㄩ杸(娆婇檺鍏х殑)
				$sql = <<<eof
					select segment_no    as dept_seqno,
					       segment_no_sz as dept_id,
					       segment_name || '(' || segment_no_sz || ')' as dept_name
...
eof;
			...
			case 'emp':
				///璇婚儴闂ㄤ笅鐨勪汉鍛�
				/* Add ename for 浼樼壒 by dennis 2010-12-06*/
				/* 鏍规嵁 define.ini 涓殑 config 璁惧畾鏉ュ喅瀹氭槸鍚︽樉绀� ename */
				$emp_name_col = isset($GLOBALS['config']['staff']['show_ename']) &&
								$GLOBALS['config']['staff']['show_ename'] == 'Y' ?
								"name_sz||' '||ename" : 'name_sz';

				// modify by dennis 20101215 绗簩涓荤鐧诲綍鏃跺幓闄ょ涓�涓荤璧勬枡
...
eof;
    ...
    * @ 淇敼涓哄彧鍦ㄤ娇鐢ㄨ�呮。涓獙璇侀偖浠�, 浣跨敤寤烘。鏃惰鍔犻偖浠�
	*
	*/
	/*
...
eof;
	...
	/**
	 * 鍥犱负鐜板湪鎵�鏈夌殑閭欢閮芥槸浠� HR_PERSONNEL涓潵锛屾墍浠ヨ繖涓壘鍥炲瘑鐮佹椂鐨勯偖浠堕獙璇佷篃鏀规垚涓�鑷�
...
eof;
    ...
    *       2. 閮ㄩ棬鏉冮檺澶氭寫鏍忎綅 dept_mask,dept_level 2006-12-06 15:16:15  by Dennis.Lan
    */
    function GetUserDataPrivileges($user_seqno,$privi_type, $where=null)
    {
        ...
        $sqlstr = <<<eof
            select seq_no,
                   ...
                   title_privilege,
                   dept_privilege,
                   /*grade_privilege, 鑱岀瓑鎺堟潈鍘绘帀浜�, modify by dennis 2008-03-25*/
                   only_self_privilege,
                   salary_privilege
...
eof;
	...
	/**
    *   @desc 妫�鏌ユ槸鍚︽湁鎺堟潈妯＄粍
    *   @param $module string, 妯＄粍鍚嶇О, ESS/MGR/BIS
...
eof;
    ...
    /**
     * 鍙� Manager 璁惧畾鐨勯璁捐繘 ESS or MSS
     *
     * @return string
